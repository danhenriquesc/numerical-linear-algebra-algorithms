...
int main(){
	...
	memcpy(U,A,sizeof(U));
	printMatrix(L, "Matrix Lower (LU'x = B)");
	printMatrix(U, "Matrix Upper' (LU'x = B)");
	...
	printMatrix(L, "Matrix Lower (LDUx = B)");
	printMatrix(D, "Matrix Diagonal' (LDUx = B)");
	printMatrix(U, "Matrix Upper' (LDUx = B)");
	...
	for(int j=1;j<=m;j++){
		...
		printMatrix(Z[j],"Matrix Z (Lz = B)","z");
		...
		printMatrix(Y[j],"Matrix Y (Dy = z)","z");
