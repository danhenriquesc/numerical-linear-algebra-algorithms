...
void printMatrix(double B[MAX_SIZE][MAX_SIZE],string,string);
...
int main(){
	...
	for(int j=1;j<=m;j++){
		...
		printMatrix(AM, "Matrix with Diagonal 1");

		for(int c=1;c<=n;c++){
			for(int l=c-1;l>=0;l--){
				mult = AM[l][c]/AM[c][c];
				for(int i=1;i<=n+1;i++){
					AM[l][i] = AM[l][i] - mult*AM[c][i];
				}
			}
		}

		printMatrix(AM, "Reduced Row Echelon Form");

		
		printMatrix(AM,"Result","x");
	}
...
void printMatrix(double B[MAX_SIZE][MAX_SIZE], string title, string var){
	cout << title << endl;

	for(int i=1;i<=n;i++){
		cout << var << i << ": " << setw(10) << B[i][n+1] << endl;
	}
